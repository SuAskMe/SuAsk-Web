...
<template>
    <div class="card-container" @click.stop="clickCard(key)">
        ...
        <div v-if="answerNum && answerNum > 0" class="card-footer">
            <el-avatar v-for="(avatar, index) in avatars" :key="index" :src="avatar" :size="32" :style="{
                marginLeft: index === 0 ? '16px' : '-16px',
                zIndex: 10 - index,
            }" class="avatar" @error="showDefaultAvatar(index)" />
            <div v-if="avatars && answerNum > avatars.length" style="font-size: 32px; color: #66b0ff">
                ...
            </div>
            <div class="text">{{ answerNum }} 个回答</div>
            <div v-if="showFavorite" class="favorite" :data-tips="isFavorite ? '取消收藏' : '收藏'"
