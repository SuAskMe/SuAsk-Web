import type { FavoriteReq, FavoriteRes, GetFavoriteByKeywordReq, GetFavoriteByKeywordRes, GetFavoriteReq, GetFavoriteRes, SearchFavoriteReq, SearchFavoriteRes } from "@/model/favorite.model";
...
export async function getFavoriteApi(req: GetFavoriteReq): Promise<GetFavoriteRes> {
    return request.get(Api.GET_FAVORITE, { params: req }).then((res) => {
        if (res) {
            return res.data
        }
    })
...
export async function getFavoriteByKeywordApi(req: GetFavoriteByKeywordReq): Promise<GetFavoriteByKeywordRes> {
