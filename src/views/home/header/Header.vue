<template>
    <div class="header">
        <div v-if="canReturn" class="back-button">
            <svg-icon icon="arrow-left" color="#71B6FF" size="30px" />
        </div>
        <div v-if="props.isQuestionPage" class="question-title">
            <p :class="isDesktop ? 'title-web' : 'title-phone'">
                {{ questionTitle }}
            </p>
            <p :class="isDesktop ? 'sub-title-web' : 'sub-title-phone'">
                {{ questionContent }}
            </p>
        </div>
        <div class="sort-and-search">
            <div class="header-item" @click="changeSort">
                ...
                <svg-icon
                    icon="sort-down"
                    :color="sortIndex == 0 ? '#808080' : '#71B6FF'"
                    size="22px"
                />
            </div>
            <div class="header-item">
                <svg-icon
                    v-if="searchIndex == 0"
                    icon="search"
                    :color="searchIndex == 0 ? '#808080' : '#71B6FF'"
                    size="22px"
                    @click="changeSearch"
                />
                <div v-else>
